...
local o = {
    ...
    ext = "",
    command_template = [[
        ffmpeg -i "$in" -ss $shift -t $duration -map 0:a? -map 0:v? -map 0:s? -c:s mov_text "$out.mp4";
    ]],
        --ffmpeg -i "$in" -ss $shift -t $duration -map 0:a -map 0:v -map 0:s -c:a copy -c:v copy -c:s copy "$out.$ext"
}
